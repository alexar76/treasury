...
# --- 2026-09 re-audit: a free-tier ceiling on a SIZED input --------------------------
#
# `enforce_free_tier` did `int(value)` and left anything non-coercible alone. That is right
# for a malformed scalar, but it also meant a ceiling on a LIST field was silently a no-op —
# and a list length is exactly what the cost scales with for the capabilities that take a
# point cloud or a graph. `free_tier_max={"points": 80}` looked like a bound and was not one.
# No existing declaration used a list field (all three were scalar ints), so sizing them is
# a pure addition.

def test_a_ceiling_on_a_list_field_bounds_its_length():
    from oracle_core.tiers import FreeTierExceeded, enforce_free_tier

    with pytest.raises(FreeTierExceeded) as exc:
        enforce_free_tier("betti.homology@v1", {"points": 80},
                          {"points": [[0.0, 0.0]] * 81})
    assert exc.value.requested == 81 and exc.value.ceiling == 80


def test_a_list_at_the_ceiling_is_allowed():
    from oracle_core.tiers import enforce_free_tier

    enforce_free_tier("betti.homology@v1", {"points": 80}, {"points": [[0.0, 0.0]] * 80})


def test_a_dict_field_is_bounded_by_its_key_count():
    from oracle_core.tiers import FreeTierExceeded, enforce_free_tier

    with pytest.raises(FreeTierExceeded):
        enforce_free_tier("x@v1", {"capacities": 2}, {"capacities": {"a": 1, "b": 2, "c": 3}})


def test_a_scalar_ceiling_is_unchanged():
    from oracle_core.tiers import FreeTierExceeded, enforce_free_tier

    enforce_free_tier("chronos.eval@v1", {"difficulty": 100_000}, {"difficulty": 100_000})
    with pytest.raises(FreeTierExceeded):
        enforce_free_tier("chronos.eval@v1", {"difficulty": 100_000}, {"difficulty": 100_001})


def test_a_value_that_is_neither_sized_nor_numeric_is_still_left_alone():
    """Rejecting malformed input stays the handler's job — its message is the better one."""
    from oracle_core.tiers import enforce_free_tier

    enforce_free_tier("x@v1", {"difficulty": 10}, {"difficulty": object()})
    enforce_free_tier("x@v1", {"difficulty": 10}, {"difficulty": None})
