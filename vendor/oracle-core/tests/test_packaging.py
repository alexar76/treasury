...
#: tree-wide checks below have nothing to look at there. The skip is narrow on purpose: it fires
#: only when the directory is absent entirely, never because an import or a dependency is
#: missing — a suite that skipped itself for a missing dependency is exactly how an earlier gap
#: in this repo stayed hidden.
IN_MONOREPO = ORACLES.is_dir()
