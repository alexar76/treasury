...
def enforce_free_tier(
    ...
    A field whose value is SIZED (a list of points, a dict of capacities) is bounded by
    its LENGTH. Without that, a ceiling on such a field was silently a no-op — ``int([...])``
    raises, and the clause below treated that as "malformed, leave it alone" — while length
    is precisely what the cost scales with for the capabilities that take a point cloud or a
    graph. A field present but neither sized nor coercible to int is still left alone:
    rejecting malformed input is the handler's job and its message is the better one.
