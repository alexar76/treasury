...
class Protocol:
    ...
    def _tool_with_metrics(self, cap: Capability) -> dict[str, Any]:
        ...
        if p50 is not None:
            ...
            # buys federation.
            # max(1, …): a measured sub-millisecond handler rounded to 0, which the hub's
            # ranking reads as "unknown" and scores like a one-second p50.
            tool["p50_latency_ms"] = max(1, int(round(p50)))
        if sr is not None:
