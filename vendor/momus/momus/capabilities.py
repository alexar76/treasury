...
class MomusRuntime:
    ...
    def import_skopos_report(self, document: dict[str, Any], *,
                             include_info: bool = False) -> dict[str, Any]:
        """Take a SKOPOS fleet-scan export into the corpus, where the fix loop can see it.

        The translator (`momus.intel.skopos_bridge`) and the route both already existed, and
        between them the findings went nowhere: the route converted the export and returned the
        count. Nothing was stored, so `GET /findings` never showed a SKOPOS finding, the autopilot
        never considered one, and `POST /remediate` answered `unknown_finding` for every id the
        route had just handed back. The bridge was real and the road stopped one metre short.

        Two things make this the same path a MOMUS-discovered finding takes, not a side door:

        * **Signed and recorded like any other.** The signature is MOMUS's — it says *MOMUS
          received and recorded this claim*, which is what the corpus's signature has always
          meant; the provenance of the observation stays in the finding itself (`probe` is
          `skopos:<category>`, and the reproducer names the scan that saw it).
        * **`seen_count` is the evidence.** The dedup key is stable per host+category+title, so a
          defect that survives N fleet scans is rediscovered N times — which is exactly the
          reproduction count the autopilot's dispatch rule asks for. That is also why SKOPOS
          sends one push per snapshot and not one per hour.

        No shortcut around `escalation_for`: a finding about an infrastructure host still routes
        to a human when it reaches `open_remediation`, imported or not.
        """
        from dataclasses import asdict

        from momus.intel.skopos_bridge import import_findings as import_skopos_findings

        findings = import_skopos_findings(document, include_info=include_info)
        server = str(document.get("server") or "")
        scan_id = f"skopos:{server}:{document.get('observed_at') or ''}"
        recorded: list[dict[str, Any]] = []
        for finding in findings:
            self.signer.sign_finding(finding)
            try:
                res = self.findings_db.record_finding(finding, scan_id=scan_id)
            except Exception as exc:  # noqa: BLE001 - one bad row must not drop the rest
                logger.warning("could not record SKOPOS finding %s: %s",
                               finding.finding_id, type(exc).__name__)
                continue
            # The corpus keeps the FIRST id for a rediscovered bug; answering with the fresh one
            # would hand SKOPOS an id that `/remediate` cannot resolve.
            canonical = res.get("finding_id") or finding.finding_id
            doc = asdict(finding)
            doc["finding_id"] = canonical
            doc["seen_count"] = res.get("seen_count")
            self._findings_by_id[canonical] = doc
            self._recent.insert(0, doc)
            recorded.append({"finding_id": canonical, "title": finding.title,
                             "severity": finding.severity, "target": finding.target,
                             "new": res.get("new"), "seen_count": res.get("seen_count")})
        del self._recent[self.RECENT_MAX:]
        while len(self._findings_by_id) > self.FINDINGS_BY_ID_MAX:
            self._findings_by_id.pop(next(iter(self._findings_by_id)), None)
        return {"server": server, "converted": len(findings), "imported": len(recorded),
                "new": sum(1 for r in recorded if r.get("new")), "findings": recorded}
    ...
    async def open_remediation(self, finding_id: str) -> dict[str, Any]:
        ...
        if ticket.route == "human-governance":
            # Two different reasons land here, and saying the wrong one sends the reader looking
            # for a self-remediation problem that is not there. A host defect is escalated because
            # no patch can close it, not because MOMUS refuses to patch itself.
            note = ("security-core finding — escalated to human governance + external verifier; "
                    "MOMUS never auto-remediates itself")
            if finding.target_kind == "host":
                note = ("host posture finding — a patch to a repository cannot close it "
                        "(see escalation_for); escalated to an operator with shell access")
            return {"ticket": ticket.to_dict(), "dispatched": False, "note": note}
