...
class PayoutGate:
    ...
    def _distinct_confirming(self, finding: Finding, verdicts: list[Verdict]) -> tuple[list[str], list[str]]:
        ...
        for v in verdicts:
            ...
            if v.finding_digest != digest:
                ...
                continue
            # WHAT did this verifier actually look at? The digest above proves the verdict
            # names this claim; it does not prove the verifier examined it. /verify/replay
            # takes target and probe from its CALLER, so without this a probe that reproduces
            # against an unrelated target (the canary always does) bought a signed "confirmed"
            # for any finding the caller named. Fail closed on an unset subject: a verdict that
            # will not say what it ran is exactly the one not to count.
            if not v.subject_target or not v.subject_probe:
                reasons.append(
                    f"verdict {v.verifier_id}: does not record which target/probe it examined "
                    "— ignored")
                continue
            if v.subject_target != finding.target or v.subject_probe != finding.probe:
                reasons.append(
                    f"verdict {v.verifier_id}: examined "
                    f"{v.subject_target!r}/{v.subject_probe!r} but the finding is about "
                    f"{finding.target!r}/{finding.probe!r} — ignored")
                continue
            if v.score < 0.5:
    ...
    def authorize(self, finding: Finding, verdicts: list[Verdict], *,
        ...
        if not settle:
            ...
            # would debit the pool twice. HELD, not PAID: PAID consumes the dedup identity and
            # the daily cap (see MomusLedger.record_decision), and nothing has moved yet. When
            # the split released nothing — Treasury down, no recipient — a PAID stamp here burned
            # the claim permanently and the retry was refused as a duplicate. authorize_split
            # promotes this decision to PAID once a share actually settles.
    ...
    def authorize_split(self, finding: Finding, verdicts: list[Verdict], *,
        ...
        if total_released > 0 and base.state == PayoutState.HELD.value:
            # Something actually moved — now the claim is spent. Re-finalising re-signs the
            # promoted decision and lets the ledger consume the dedup key and the daily cap
            # exactly once, on the run that paid.
            base.state = PayoutState.PAID.value
            base.reasons.append(f"promoted to paid — ${total_released} released across shares")
            self._finalize(base, finding.scanner_pubkey or self._keys.scanner_pubkey, sign=True)
        released_state = base.state
