...
#: Categories a patch can actually close on a MACHINE finding. The fix loop edits a component's
#: repository and redeploys its container, so "this dependency has a published CVE" is squarely
#: in its hands — that is a line in a lockfile. "Postgres is bound to 0.0.0.0 on this host",
#: "fail2ban is not running", "this certificate expires in six days" are properties of the
#: machine, and no patch to any repo changes them.
_HOST_FIXABLE_CATEGORIES = frozenset({"supply-chain"})


def escalation_for(component: str, target_kind: str = "", category: str = "") -> str:
    ...
    Host findings (SKOPOS's fleet scan) escalate too, unless they are supply-chain. This is not
    caution for its own sake: dispatching "the database listens on every interface" to a fixer
    that can only author a patch spends an agent run producing something the deploy gate is then
    obliged to reject, and the defect is still there afterwards. A person with shell access fixes
    it in a minute. The finding still becomes a signed, blamed, routed ticket — which is the whole
    difference from where those findings used to end, in a dashboard gauge.
    """
    if component in _INFRA_COMPONENTS or target_kind in ("self", "meta"):
        return "human-governance"
    if target_kind == "host" and category not in _HOST_FIXABLE_CATEGORIES:
        return "human-governance"
    return "auto"
...
def open_ticket(finding: Finding, signer: FindingSigner) -> RemediationTicket:
    ...
    route = escalation_for(finding.target, finding.target_kind, finding.category)
    blame = Blame(
