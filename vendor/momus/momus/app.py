...
def build_app(runtime: MomusRuntime | None = None) -> FastAPI:
    ...
    def extra(app: FastAPI, proto) -> None:
        ...
        @app.post("/skopos/report", response_model=None)
        async def skopos_report(body: dict, request: Request):
            """SKOPOS pushes what its fleet scan found; MOMUS turns it into findings.

            SKOPOS finds real defects on the fleet — a database bound to 0.0.0.0, an
            expiring certificate, a container running as root — and then only DRAWS
            them: `skopos/skopos/ui_security.py` renders a gauge and a banner and the
            trail ends there. Meanwhile every mechanism for actually fixing a defect
            (sign it, blame it, route it, patch it, re-probe it before accepting the
            fix) already lives here and is target-agnostic.

            OPERATOR-ONLY, and for the same reason /warden/report is not: this writes
            into the queue that opens remediation tickets. An unauthenticated caller
            could otherwise invent findings against any host MOMUS knows.

            Imported findings get NO special treatment — in particular no shortcut
            around `escalation_for`, so a finding about a security-core host still
            routes to a human instead of collecting an automated patch.
            """
            _limit(scan_limiter, request)          # writes, so the tighter bucket
            _require_operator(request)
            try:
                # RECORDED, not merely converted. This route used to translate the export and
                # return the count, which left the findings in a local variable that went out of
                # scope: nothing reached the corpus, so no imported finding was ever visible to
                # GET /findings, to the autopilot, or to POST /remediate. See
                # MomusRuntime.import_skopos_report.
                return runtime.import_skopos_report(
                    body or {}, include_info=bool((body or {}).get("include_info")))
