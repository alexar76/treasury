    ...
    async def run(self, target: Target, ctx: ProbeContext, discovery: dict[str, Any]) -> list[ProbeResult]:
        ...
        if not genuine_ok:
            return [ProbeResult(
                ...
                # A probe that says "your signature does not verify" without saying WHAT is
                # signed is asking whoever fixes it to guess an interop contract. Three
                # autonomous repair attempts in a row each produced a plausible signature over a
                # different string, and the gate rejected all three. The canonical is derived
                # from the manifest the target itself publishes, so naming it discloses nothing
                # the reader cannot already compute — it only saves them from guessing.
                response_summary=(
                    "Sign Ed25519 over oracle_core.signing.Signer.manifest_canonical(manifest) — "
                    "IMPORT it, do not reimplement it: `from oracle_core.signing import Signer` "
                    "is installed in this service's image, and a second copy of an interop "
                    "canonical drifts the day the first one gains a field. It is NOT "
                    "json.dumps(manifest): it is "
                    "'capabilities_count:<n>|generated_at:<iso>|protocol_version:<v>"
                    "|tools_hash:<...>|by_hub_hash:<...>'. Publish the matching public key in "
                    f"both signature.public_key and .well-known. Computed: {canonical[:120]}"
                ),
                # Named, not left to a regular expression to find in the sentence above.
                reference_artifacts=("oracles/core/oracle_core/signing.py",),
                status_code=200,
            )]
