...
class ProbeResult:
    ...
    reproducer: str = ""
    #: Files whoever fixes this needs to READ, named by the probe that asserts the contract.
    #:
    #: The fixer's retrieval was a regular expression looking for dotted module names in this
    #: probe's prose. It works — measured, it pulls the right 8kB file for the manifest probe —
    #: and it works by luck: it fires only when a probe happens to phrase the contract as
    #: `package.module.Class`. A probe knows exactly which file defines what it is asserting,
    #: so it can say so instead of hoping a sentence parses.
    #:
    #: Repo-relative paths. The fixer still applies its read policy to every one of them, so
    #: naming a credential or the auditor's own source here gets refused, not served.
