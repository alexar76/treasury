...
class Evidence:
    ...
    #: Files whoever fixes this needs to READ, named by the probe rather than inferred from
    #: its prose. Paths only — the contents travel no further than the fixer's own mount, and
    #: the fixer applies its read policy to each one, so a probe naming a credential gets a
    #: refusal rather than a disclosure.
    #:
    #: Safe to add to a signed document: `compute_dedup_key` is built from target, probe,
    #: category and status code, so the corpus keeps its sighting history across this change.
...
class Verdict:
    ...
    created_at: str = field(default_factory=_now_z)
    verifier_pubkey: str = ""
    #: WHAT this verifier actually examined. Inside canonical(), so inside the signature.
    #:
    #: `finding_digest` says which CLAIM is being judged; it does not say what the verifier
    #: ran. A replaying instance is handed target and probe by its caller (it cannot look the
    #: finding up — it is a different instance, that is the point), so a caller could ask for
    #: a probe that reliably reproduces somewhere unrelated (momus/canary exists precisely to
    #: always reproduce) and receive a signed "confirmed" for any finding it named. The payout
    #: gate then saw a valid signature, a matching digest and an independent key, and counted
    #: it. The sibling FixVerdict already carries target/probe for the same reason, with the
    #: comment that "a verdict that does not say which build it looked at is ambiguous exactly
    #: where a deploy gate must not be" — the same is true where a payout gate must not be.
    subject_target: str = ""
    subject_probe: str = ""
    signature: dict[str, Any] = field(default_factory=dict)
