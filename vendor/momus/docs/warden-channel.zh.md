...
MOMUS 负责找出敌意的第三方 MCP 服务器。[WARDEN](https://github.com/alexar76/argus) —— 每一个 ARGUS
安装内部的那道防火墙 —— 决定它的所有者可以接触哪些服务器。在这条通道存在之前，这两件事从未相遇：
红队不断找到东西，而蓝队从来没听说过。
...
flowchart LR
  ...
  subgraph momus["MOMUS —— 唯一的验证方"]
    Q["上报队列<br/>未验证的线索<br/><i>仅运营者可读</i>"]
...
所以：**任何人都可以上报；只有运营者可以读这个队列。** 这是通过核查线上部署发现的，不是通过读
代码 —— 代码看起来没什么问题。
...
那个显而易见的下一步 ——「一收到上报，就去扫描那个 URL」—— 会把 MOMUS 变成一个开放的扫描中继：
任何人只要 POST 一个主机名，就能把一支带签名、资源充足的红队瞄准互联网上的任意主机。那是一件
流量放大武器，也是别人家的一次事故。探测始终被限制在**由运营者注册过的目标**上；一条上报最多只能
把一个候选项排进队列，等待那个决定。

已在线上核实：携带 `"scan": true` 和 `"target_url"` 的上报被作为线索接收，并且什么也没有启动。
...
就能永久占住运营者的注意力。上报方给出的严重程度在入口处被压到上限 `high`；`critical` 是靠同一台
服务器收到两份彼此独立的上报**挣来的**。

去重身份只包含**服务器，别无其他** —— 不含上报方，也不含工具列表。把工具算进去曾经是一个 bug，
是线上核查暴露出来的：不同的安装查询的是不同的工具子集，于是同一台敌意服务器变成了好几条互不相关
的线索，每条计数都是 1，`corroborated: 0` —— 而实际上确实有两个安装上报过它。这和曾经把一个易变的
...
喂进假的拒绝项。给每一个个人智能体开一个入站 A2A 端口，和当初为[部署用节点智能体](found-and-fixed.md)
所否决掉的，是同一个反模式。
...
| `MOMUS_WARDEN_REPORTS` | MOMUS | 关闭 | 接收来自现场的上报 |
...
| `momus/tests/test_warden_reports.py`（27） | 上报入口校验、四层防诽谤、路由巡检、「没有任何推理组件读这个队列」这条不变式、佐证 |
