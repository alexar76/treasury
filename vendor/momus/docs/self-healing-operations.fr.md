...
## Constats venus de SKOPOS

SKOPOS scanne le parc et trouve de vrais défauts — une base liée à `0.0.0.0`, un
certificat qui expire, un conteneur en root, fail2ban à l'arrêt. Jusqu'ici il se
contentait de les **dessiner** : `skopos/skopos/ui_security.py` affiche une jauge, une
bannière et un badge, et la piste s'arrêtait là. Personne ne les consommait, donc chacun
attendait qu'une personne le remarque. Son « tableau de remédiation » montre des travaux
que le chef d'orchestre y pousse ; SKOPOS n'en a jamais créé.

SKOPOS les envoie désormais à `POST /skopos/report` sur MOMUS, et ce sont dès lors des
constats **ordinaires** : la route **les enregistre dans le corpus**, donc ils
apparaissent dans `GET /findings`, l'autopilote les examine et `POST /remediate` résout
leurs ids. `momus/momus/intel/skopos_bridge.py` traduit gravité et catégorie, garde une
clé de déduplication stable d'un scan à l'autre (un scan horaire d'un port ouvert est un
constat, pas vingt-quatre) et note comme reproducteur le scan qui l'a vu.

**En production les deux sont sur des machines différentes, et le sens s'inverse.** MOMUS
n'écoute que sur la boucle locale et son bord public répond `404` à toute route qui le fait
agir — `/skopos/report` comprise — donc l'hôte SKOPOS ne peut pas y poster, jeton ou pas.
SKOPOS publie à la place un `GET /security/export` en lecture seule, derrière son propre
jeton, et l'hôte MOMUS le récupère puis poste via `127.0.0.1`. Les lectures sortent de
SKOPOS ; les écritures restent dans MOMUS ; aucune machine n'obtient de shell ni
d'identifiant sur l'autre.

L'appelant est `skopos/skopos/security/momus_push.py` : via `skoposctl momus-push`, en
démon autonome, ou une fois par tick de l'autopilote. Il est éteint tant que
`SKOPOS_MOMUS_PUSH=1` n'est pas posé, et c'est un **interrupteur distinct de
`AUTOPILOT_ENABLED`** : signaler une observation n'est pas le même acte que laisser la
boucle modifier la production, et les preuves qu'un envoi exige doivent commencer à
s'accumuler avant que quiconque arme la boucle. Il envoie **un push par instantané** :
`seen_count` veut dire « N scans distincts ont retrouvé ce défaut », et un émetteur sur
minuterie le fabriquerait à partir d'une seule observation.

Ils suivent ensuite la même route que ce que MOMUS trouve lui-même :

| Route | Ce qui se passe |
|---|---|
| `auto` | correction → nouveau test → redéploiement, à condition que la sonde ne reproduise plus |
| `human-governance` | une personne, plus un vérificateur exploité indépendamment |

Le choix revient à `escalation_for`, **inchangé et non consulté par le pont**. Un constat
SKOPOS visant un hôte du cœur de sécurité remonte à un humain exactement comme un constat
natif : importer un constat n'est pas un moyen de poser un correctif automatique sur une
machine d'infrastructure. La gravité `info` est écartée sauf demande explicite.

**La plupart remontent à un humain, et prétendre le contraire serait la version
malhonnête.** La boucle corrige un défaut en éditant le dépôt d'un composant et en
redéployant son conteneur. « Postgres écoute sur `0.0.0.0` sur cette machine », « fail2ban
ne tourne pas », « ce certificat expire dans six jours » sont des propriétés de la
*machine* : aucun correctif dans un dépôt ne les ferme, donc en envoyer un achète un
correctif rejeté et une exécution d'agent dépensée pendant que le port reste ouvert. Ils
deviennent des tickets signés, avec blâme et route, pour un opérateur ayant un shell — ce
qui reste toute la différence avec l'endroit où ils finissaient avant, dans une jauge.
L'exception est **supply-chain** (`packages`, `project`) : une dépendance vulnérable *est*
une ligne dans un lockfile, et correction → nouveau test → redéploiement est exactement ce
qu'il lui faut.

Le point d'entrée exige le jeton opérateur. Il écrit dans la file qui ouvre les tickets ;
sans cela, un appelant non authentifié pourrait inventer des constats sur n'importe quel
hôte connu de MOMUS.
