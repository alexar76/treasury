...
MOMUS находит враждебные сторонние MCP-серверы. [WARDEN](https://github.com/alexar76/warden) — MCP-файрвол,
поставляемый как отдельный пакет `@aimarket/warden` и работающий в каждой установке ARGUS, — решает, к каким серверам его владельцу можно подключаться. Пока этого
