...
**Lo que NO ha ocurrido:** la AI-Factory nunca ha escrito de forma autónoma un parche que arreglara un
bug real. El cliente de la Factory funciona en dry-run; el paso de «arreglo» en la cadena en vivo es
un cambio de interruptor en el montaje de prueba. La *fontanería* del bucle es real y está demostrada
de extremo a extremo — la *autoría de la corrección* todavía no es autónoma. Lo decimos sin rodeos
...
que no tiene ninguna clave con la que falsificarlo. La allowlist (lista blanca) es **local** — la
...
- **tareas sin autenticar** → se exige `SKOPOS_A2A_TOKEN`, fail-closed (denegar por defecto) fuera de
  dry-run;
