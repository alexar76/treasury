...
## Findings that come from SKOPOS

SKOPOS scans the fleet and finds real defects — a database bound to `0.0.0.0`, a
certificate about to expire, a container running as root, fail2ban not running. Until
now it only **drew** them: `skopos/skopos/ui_security.py` renders a gauge, a banner and
a sidebar badge, and the trail ended there. Nothing consumed them, so each one waited
for a person to notice. Its "remediation board" is a display of jobs the conductor
pushes *in*; SKOPOS never created any.

SKOPOS now pushes those findings to `POST /skopos/report` on MOMUS, and from that point
they are **ordinary findings**: the route **records them in the corpus**, so they appear
on `GET /findings`, the autopilot considers them, and `POST /remediate` resolves their
ids. `momus/momus/intel/skopos_bridge.py` maps severity and category, keeps a dedup key
stable across re-scans (an hourly scan of one open port is one finding, not twenty-four),
and records the scan that saw it as the reproducer.

**In production the two are on different machines, and the direction is inverted.** MOMUS
binds to loopback and its public edge answers `404` for every route that makes it act —
`/skopos/report` among them — so the SKOPOS host cannot post, token or no token. Instead
SKOPOS publishes a read-only `GET /security/export` behind its own token, and the MOMUS
host pulls that and posts over `127.0.0.1`. Reads leave SKOPOS; writes stay inside MOMUS;
neither machine gains a shell or a credential on the other.

The caller is `skopos/skopos/security/momus_push.py` — as `skoposctl momus-push`, as a
standalone daemon, or once per autopilot tick. It is off until `SKOPOS_MOMUS_PUSH=1`,
which is a **different switch from `AUTOPILOT_ENABLED`**: reporting an observation is not
the same act as letting the loop change production, and the evidence a dispatch needs has
to start accumulating before anyone arms the loop. It sends **one push per snapshot** —
`seen_count` is supposed to mean "N separate scans rediscovered this", and a pusher on a
timer would manufacture that out of a single observation.

They then take the same road as anything MOMUS found itself:

| Route | What happens |
|---|---|
| `auto` | fix → retest → redeploy, gated on the probe no longer reproducing |
| `human-governance` | a person, plus an independently-operated verifier |

Which one applies is decided by `escalation_for`, **unchanged and not consulted by the
bridge**. A SKOPOS finding about a security-core host escalates to a human exactly like
a native one — importing a finding is not a way to get an automated patch onto an infra
box. `info` severity is dropped unless asked for, because a fix round costs an agent run
and "sshd is listening on 22" is not worth one.

**Most of them escalate, and pretending otherwise would be the dishonest version.** The
loop fixes a defect by editing a component's repository and redeploying its container.
"Postgres is bound to `0.0.0.0` on this machine", "fail2ban is not running", "this
certificate expires in six days" are properties of the *machine*: no patch to any repo
closes them, so dispatching one buys a rejected patch and a spent agent run while the
port stays open. Those become signed, blamed, routed tickets for an operator with shell
access — which is still the whole difference from where they used to end, in a gauge. The
exception is **supply-chain** (`packages`, `project`): a vulnerable dependency *is* a line
in a lockfile, and fix → retest → redeploy is exactly what that needs.

The endpoint is operator-token only. It writes into the queue that opens tickets, so an
unauthenticated caller could otherwise invent findings against any host MOMUS knows.
