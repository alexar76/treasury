...
SKOPOS 扫描机群并找出真实缺陷——绑定到 `0.0.0.0` 的数据库、即将过期的证书、以 root
运行的容器、未运行的 fail2ban。此前它只是把它们**画出来**：
`skopos/skopos/ui_security.py` 渲染仪表、横幅和侧栏徽章，线索到此为止。没有任何环节
消费它们，于是每一条都在等人注意到。它的"修复看板"展示的是指挥者推送进来的作业；
SKOPOS 自己从未创建过任何一条。

现在 SKOPOS 把这些发现推送到 MOMUS 的 `POST /skopos/report`，此后它们就是**普通的
发现**：该路由会**把它们写入语料库**，因此它们出现在 `GET /findings` 中，自动驾驶会
考虑它们，`POST /remediate` 也能解析它们的 id。`momus/momus/intel/skopos_bridge.py`
映射严重级别与类别，并在多次扫描之间保持去重键稳定（每小时扫描同一个开放端口是一条
发现，而不是二十四条），同时把看到它的那次扫描记录为复现方式。

**生产环境里两者在不同机器上，方向是反过来的。** MOMUS 只绑定回环，它的公开入口对每一个会让
它「动作」的路由都返回 `404`——`/skopos/report` 也在其中——所以 SKOPOS 那台机器根本推不过去，
有没有令牌都一样。取而代之的是：SKOPOS 用自己的令牌公开一个只读的 `GET /security/export`，由
MOMUS 所在的机器去拉取，再经 `127.0.0.1` 推送。读离开 SKOPOS，写留在 MOMUS 内部，两台机器都
不会获得对方的 shell 或凭据。

调用方是 `skopos/skopos/security/momus_push.py`：可以作为 `skoposctl momus-push`、独立
守护进程，或在自动驾驶每个 tick 中执行一次。未设置 `SKOPOS_MOMUS_PUSH=1` 之前它是关闭
的，而这是**与 `AUTOPILOT_ENABLED` 不同的开关**：上报一次观察，和允许该闭环改动生产，
并不是同一件事；而派发所需的证据必须在有人武装该闭环之前就开始积累。它**每个快照只推送
一次**：`seen_count` 的含义是「N 次独立扫描重新发现了它」，而按定时器推送的程序会把单次
观察伪造成这个数字。

之后它们与 MOMUS 自己发现的走同一条路：

| 路由 | 会发生什么 |
|---|---|
| `auto` | 修复 → 复测 → 重新部署，以探针不再复现为前提 |
| `human-governance` | 由人处理，外加一个独立运营的验证方 |

由 `escalation_for` 决定走哪条，**未作改动，桥接也不参与该判断**。针对安全核心主机的
SKOPOS 发现会像原生发现一样上报给人：导入发现不能成为把自动补丁送上基础设施机器的
途径。除非明确要求，`info` 级别会被丢弃，因为一轮修复要花费一次代理运行。

**其中大多数都会上报给人，假装并非如此才是不诚实的版本。** 该闭环修复缺陷的方式，是编辑
组件的代码仓库并重新部署其容器。「这台机器上的 Postgres 监听 `0.0.0.0`」「fail2ban 没有
运行」「这张证书六天后过期」都是*机器*的属性：任何仓库里的补丁都关不掉它们，因此派发一条
这样的发现，买到的是一个被拒绝的补丁和一次白花的代理运行，而端口依旧开着。它们会变成带
签名、带归因、带路由的工单，交给有 shell 权限的运维——这与它们过去的归宿（一个仪表盘上的
指针）相比，仍然是全部的差别。例外是 **supply-chain**（`packages`、`project`）：有漏洞的
依赖*就是*锁文件里的一行，而修复 → 复测 → 重新部署正是它需要的。

该端点仅接受运维令牌。它写入的是开具工单的队列；否则未经认证的调用方就能针对 MOMUS
已知的任何主机凭空捏造发现。
