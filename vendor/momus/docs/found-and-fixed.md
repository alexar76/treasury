...
**What has NOT happened:** the AI-Factory has never autonomously authored a patch that fixed a real
bug. The Factory client runs in dry-run; the "fix" step in the live chain is a fixture flip. The
loop's *plumbing* is real and proven end to end — the *fix authorship* is not yet autonomous. Said
plainly so nobody reads more into the demo than it earns.
...
- `MomusClient` treats a 200 body without a boolean `fixed` as `inconclusive` and names the real
  cause ([`clients.py`](../../skopos/skopos/remediation/clients.py));
- the conductor **stops** on an inconclusive gate instead of looping: `"deploy gate could not run —
  ...
  attempt budget only buys a wrong escalation ([`conductor.py`](../../skopos/skopos/remediation/conductor.py)).
