...
**这件事现在已经发生了 —— 2026-08-27。** AI-Factory 写出了一个真正修好运行中服务缺陷的补丁，机群
完成构建，MOMUS 通过部署闸门放行，节点智能体上线，MOMUS 再对线上服务确认修复：全程 5 分 2 秒，
回路中没有人。完整记录、diff 与七项独立核验见 [first-self-heal.zh.md](first-self-heal.zh.md)，
其中也包括只有真实运行才暴露出来的七个缺陷。

仍有两条界限成立，也正因如此这个演示不该被读得比它本身更大：目标是[金丝雀](../canary/README.md)
测试装置，而智能体的允许清单恰好只有这一个服务。一份已签名的 `fixed` 裁决证明该发现不再复现；它
并不证明补丁是*好的*——这正是那个分支存在的理由，而合并始终是人的决定。
