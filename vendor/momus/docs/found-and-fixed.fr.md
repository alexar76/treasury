...
| **Les propres sondes de MOMUS** | des violations de contrat dans le banc d'essai [canari](../canary/README.md) | détection entièrement autonome |

**Cela a désormais eu lieu — 2026-08-27.** L'AI-Factory a écrit un correctif qui a réparé un vrai
défaut dans un service en fonctionnement, la flotte l'a compilé, MOMUS l'a soumis à sa porte de
déploiement, un agent de nœud l'a publié et MOMUS a confirmé la correction contre le service en
production : 5 minutes 2 secondes, sans humain dans la boucle. Le compte rendu complet, le diff et
sept vérifications indépendantes sont dans [first-self-heal.fr.md](first-self-heal.fr.md), y compris
les sept défauts que seule une exécution réelle a révélés.

Deux limites tiennent toujours, et c'est pourquoi la démo ne doit pas être lue pour plus qu'elle
n'est : la cible était le banc d'essai du [canari](../canary/README.md), et la liste de l'agent
contient exactement ce seul service. Un verdict `fixed` signé prouve que le constat a cessé de se
reproduire ; il ne prouve pas que le correctif soit *bon*, d'où l'existence de la branche et le fait
que la fusion reste la décision d'un humain. Dit clairement, pour que personne ne lise
...
Treasury depuis un processus non privilégié sur le réseau Docker partagé.
...
Les chiffres de l'observateur lui-même pour cette exécution :
