...
**Чего НЕ произошло:** AI-фабрика ни разу не написала автономно патч, который починил бы настоящий
баг. Клиент фабрики работает в dry-run; шаг «фикс» в живой цепочке — это переключение фикстуры.
*Обвязка* цикла настоящая и доказана от начала до конца, а вот *авторство правки* пока не автономно.
Сказано прямо, чтобы из демо никто не вычитал больше, чем оно заслуживает.
...
- `MomusClient` считает тело 200 без булева `fixed` результатом `inconclusive` и называет настоящую
  причину ([`clients.py`](../../skopos/skopos/remediation/clients.py));
