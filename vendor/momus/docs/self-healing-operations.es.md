...
## Hallazgos que vienen de SKOPOS

SKOPOS escanea la flota y encuentra defectos reales — una base de datos escuchando en
`0.0.0.0`, un certificado por vencer, un contenedor como root, fail2ban detenido. Hasta
ahora sólo los **dibujaba**: `skopos/skopos/ui_security.py` pinta un medidor, un aviso y
una insignia, y ahí terminaba el rastro. Nadie los consumía, así que cada uno esperaba a
que una persona lo notara. Su «tablero de remediación» muestra trabajos que el director
empuja *hacia dentro*; SKOPOS nunca creó ninguno.

Ahora SKOPOS los envía a `POST /skopos/report` en MOMUS y desde ahí son hallazgos
**ordinarios**: la ruta **los registra en el corpus**, así que aparecen en
`GET /findings`, el autopiloto los considera y `POST /remediate` resuelve sus ids.
`momus/momus/intel/skopos_bridge.py` traduce severidad y categoría, mantiene una clave de
deduplicación estable entre escaneos (un escaneo cada hora de un puerto abierto es un
hallazgo, no veinticuatro) y registra como reproductor el escaneo que lo vio.

**En producción están en máquinas distintas y la dirección se invierte.** MOMUS escucha
solo en loopback y su borde público responde `404` a toda ruta que lo haga actuar, incluida
`/skopos/report`: el host de SKOPOS no puede publicar ahí, con token o sin él. En su lugar
SKOPOS expone `GET /security/export`, de solo lectura y con su propio token, y el host de
MOMUS lo consulta y publica por `127.0.0.1`. Las lecturas salen de SKOPOS; las escrituras se
quedan dentro de MOMUS; ninguna máquina obtiene shell ni credenciales en la otra.

Quien llama es `skopos/skopos/security/momus_push.py`: como `skoposctl momus-push`, como
demonio propio, o una vez por tick del autopiloto. Está apagado hasta
`SKOPOS_MOMUS_PUSH=1`, que es un **interruptor distinto de `AUTOPILOT_ENABLED`**:
informar de una observación no es el mismo acto que dejar que el bucle cambie producción,
y la evidencia que un despacho necesita tiene que empezar a acumularse antes de que
alguien arme el bucle. Envía **un push por instantánea**: `seen_count` significa «N
escaneos distintos volvieron a encontrar esto», y un emisor por temporizador lo fabricaría
a partir de una sola observación.

Después recorren el mismo camino que lo que MOMUS encuentra por su cuenta:

| Ruta | Qué ocurre |
|---|---|
| `auto` | arreglo → reprueba → redespliegue, condicionado a que la prueba ya no reproduzca |
| `human-governance` | una persona, más un verificador operado de forma independiente |

Cuál aplica lo decide `escalation_for`, **sin cambios y sin que el puente lo consulte**.
Un hallazgo de SKOPOS sobre un host del núcleo de seguridad escala a una persona igual
que uno nativo: importar un hallazgo no es una vía para colocar un parche automático en
una máquina de infraestructura. La severidad `info` se descarta salvo petición expresa,
porque una ronda de arreglo cuesta una ejecución de agente.

**La mayoría escala, y fingir lo contrario sería la versión deshonesta.** El bucle arregla
un defecto editando el repositorio de un componente y redesplegando su contenedor.
«Postgres escucha en `0.0.0.0` en esta máquina», «fail2ban no está corriendo», «este
certificado caduca en seis días» son propiedades de la *máquina*: ningún parche a ningún
repositorio las cierra, así que despachar una compra un parche rechazado y una ejecución
de agente gastada mientras el puerto sigue abierto. Se convierten en tickets firmados, con
culpa y ruta, para un operador con acceso al shell — que sigue siendo toda la diferencia
con donde acababan antes, en un indicador. La excepción es **supply-chain** (`packages`,
`project`): una dependencia vulnerable *es* una línea en un lockfile, y arreglo → reprueba
→ redespliegue es exactamente lo que necesita.

El endpoint exige el token de operador. Escribe en la cola que abre tickets; sin eso,
cualquiera podría inventar hallazgos contra cualquier host que MOMUS conozca.
