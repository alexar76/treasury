...
def _protocol_reference_run_py() -> pathlib.Path:
    """Locate the protocol's conformance runner in monorepo or satellite layout.

    Monorepo: sibling ``aimarket-protocol/conformance/run.py`` (``Dockerfile`` COPYs it).
    GitHub satellite: ``vendor/protocol-conformance/run.py`` (mirror + ``Dockerfile.standalone``).
    """
    here = pathlib.Path(__file__).resolve()
    # tests/ → package root (momus/ in monorepo, satellite checkout root on GitHub)
    pkg_root = here.parents[1]
    # momus/tests/ → monorepo root when nested under aicom/
    mono_root = here.parents[2]
    candidates = [
        mono_root / "aimarket-protocol" / "conformance" / "run.py",
        pkg_root / "vendor" / "protocol-conformance" / "run.py",
        pkg_root / "aimarket-protocol" / "conformance" / "run.py",
    ]
    for path in candidates:
        if path.is_file():
            return path
    raise FileNotFoundError(
        "protocol conformance run.py not found (expected monorepo sibling "
        "aimarket-protocol/conformance or satellite vendor/protocol-conformance)"
    )
...
def test_the_shipped_reference_is_the_protocols_own_and_not_a_copy_of_oracle_core():
    ...
    ref = _protocol_reference_run_py()
