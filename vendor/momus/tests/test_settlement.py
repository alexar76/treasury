...
def test_base_prepares_unsigned_call_never_broadcasts():
    # fallback="held" pins the pure prepare-and-stall stance. The DEFAULT rail falls back to the
    # sandbox instead (see tests/test_settlement_rails.py) — but it still never broadcasts.
    b = SettlementBackend(SettlementMode.BASE, "base", splitter="0x" + "cd" * 20, fallback="held")
