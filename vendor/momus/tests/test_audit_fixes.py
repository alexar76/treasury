...
def test_split_settles_each_share_exactly_once(tmp_path, scanner, verifier_a, verifier_b):
    ...
    out = gate.authorize_split(
        f, [verifier_a.sign_verdict(Verdict(f.finding_id, finding_digest(f), "confirmed", "replay", 0.95, "r", "a")),
            verifier_b.sign_verdict(Verdict(f.finding_id, finding_digest(f), "confirmed", "replay", 0.95, "r", "b"))],
