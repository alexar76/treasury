...
def test_scanner_cannot_self_verify(gate, scanner):
    ...
    self_v = scanner.sign_verdict(Verdict(f.finding_id, finding_digest(f), "confirmed", "self", 1.0, "r", "sc", subject_target=f.target, subject_probe=f.probe))
...
def test_small_order_verifier_key_rejected(gate, scanner):
    ...
    fake = Verdict(f.finding_id, finding_digest(f), "confirmed", "x", 1.0, "r", "fake", subject_target=f.target, subject_probe=f.probe)
...
# --- 2026-09 re-audit: a verdict must say WHAT it examined --------------------------
#
# /verify/replay takes finding_id, finding_digest, target AND probe from its caller — it has
# to, because the replaying instance is a different instance and cannot look the finding up.
# The Verdict it signed recorded only the first two. So a caller could ask a peer to re-run a
# probe that reliably reproduces against an unrelated target (momus/canary exists precisely
# to always reproduce) and get back a signed "confirmed" carrying any finding_id/digest it
# named. Every gate check then passed: right digest, valid signature, independent key.

def test_a_verdict_that_examined_a_different_subject_does_not_count(gate, scanner, verifier_a):
    """The transplant that the digest check cannot see."""
    f = _finding(scanner)
    elsewhere = verifier_a.sign_verdict(Verdict(
        f.finding_id, finding_digest(f), "confirmed", "replay", 0.95, "r",
        verifier_a.pubkey[:6],
        # Correct claim id, correct digest, valid independent signature — and it re-ran a
        # probe against the canary, which always reproduces.
        subject_target="momus-canary", subject_probe="always_fails",
    ))
    distinct, reasons = gate._distinct_confirming(f, [elsewhere])
    assert distinct == [], "a verdict about another target/probe was counted"
    assert any("examined" in r for r in reasons), reasons


def test_a_verdict_that_records_no_subject_does_not_count(gate, scanner, verifier_a):
    """Fail closed: the attacker writes the document, so an omitted field is a chosen field."""
    f = _finding(scanner)
    silent = verifier_a.sign_verdict(Verdict(
        f.finding_id, finding_digest(f), "confirmed", "replay", 0.95, "r",
        verifier_a.pubkey[:6],
    ))
    distinct, reasons = gate._distinct_confirming(f, [silent])
    assert distinct == []
    assert any("does not record" in r for r in reasons), reasons


def test_an_honest_verdict_about_this_finding_still_counts(gate, scanner, verifier_a):
    """The bound must not be an outage: the legitimate case has to keep working."""
    f = _finding(scanner)
    honest = _confirm(verifier_a, f)
    distinct, reasons = gate._distinct_confirming(f, [honest])
    assert distinct == [verifier_a.pubkey], reasons


def test_the_subject_is_inside_the_signature(scanner, verifier_a):
    """Recording it is worthless if it can be edited in flight."""
    f = _finding(scanner)
    v = _confirm(verifier_a, f)
    assert "subject_target" in v.canonical() and "subject_probe" in v.canonical()
    v.subject_target = "momus-canary"          # tamper after signing
    from momus.economics import verify_document_signature
    assert not verify_document_signature(v.canonical(), v.signature, v.verifier_pubkey)
