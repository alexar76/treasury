...
#
# The port is written `:9085`, not `9085`. A bare four-digit run matches anything — and these
# bodies carry base64 signatures and hex digests that are, by construction, random, so it did:
# measured at roughly 1 body in 300, which is a test that fails a few times a week for a reason
# that has nothing to do with disclosure. `:9085` still catches the leak in every form it could
# actually take (`hub:9085`, `10.0.0.5:9085`, `{"port":9085}`) because a port follows a colon,
# and base64 has no colons.
...
async def test_every_leak_token_is_something_a_record_really_carries(momus):
    """A "not in body" assertion proves nothing about a token no record ever holds.

    Which is the trap in narrowing one: the bare `9085` above was flaky because four
    digits match random base64, and the cheapest way to stop the flake would have been
    to narrow it into something unmatchable — a green test guarding nothing. So pin the
    other half: every token LEAKS refuses to serve must be present on the raw advisory,
    or the refusal is vacuous.
    """
    _client_unused, runtime = momus
    raw = json.dumps(runtime.bulletin.publish(_finding(runtime)).raw_dict())
    for leak in LEAKS:
        if leak == '"n": 1000':
            continue  # a JSON-spacing variant of n=1000: an alternate rendering to
            # guard against, not a string the record itself holds.
        assert leak in raw, f"{leak!r} is on no record, so refusing to serve it proves nothing"
