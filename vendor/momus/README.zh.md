...
- **PyPI：** `aimarket-momus` · **生产主机：** 服务器 2（`modeldev`），发布于 `redteam.modelmarket.dev`
...
>
> **已部署的合约并不等于已启用支付。** [`BountySplitter`](../contracts/evm/src/BountySplitter.sol) **已**部署在 Base mainnet
...
（`contracts/evm/test/BountySplitter.t.sol`）。生态系统的完整地址列表：[`docs/onchain-journal.md`](../docs/onchain-journal.md)。
