...
/* MOMUS — adversarial-audit satellite. A single SPA with three views wired to a
 * hash route (no router library): '#/live' shows the live scanner console,
 * '#/bulletin' shows the published advisory record, and everything else shows
 * the landing page. An unknown hash falls back to the landing rather than to a
 * 404 view — a mistyped fragment should land somewhere useful. */

type Route = "landing" | "live" | "bulletin";
...
function routeFromHash(): Route {
  ...
  if (raw === "bulletin") return "bulletin";
  return "landing";
}
    ...
    <div className="app">
      <header className="nav">
        ...
        <nav className="nav-links">
          ...
          <button
            className={route === "bulletin" ? "nav-link active" : "nav-link"}
            onClick={() => go("bulletin")}
          >
            {t("nav.bulletin", undefined, "Bulletin")}
          </button>
          <a
      ...
      <main>
        {route === "live" ? (
          ...
          <ErrorBoundary>
            {/* The scanner key is also the key the signed index verifies under, so the page can
                show what to pin without a second request. */}
            <Bulletin scannerPubkey={scannerPubkey} />
          </ErrorBoundary>
        ) : (
          <>
