// Verify the MOMUS → WARDEN channel against a LIVE deployment, using WARDEN's OWN code.
//
...
//
// It imports the canonicalizer out of @aimarket/warden — the firewall package the consumer runs — so
// the canonical bytes come from the CONSUMER's implementation, and verifies with node:crypto exactly as
// WARDEN does. Requires that package to be built (`cd warden && npm run build`).
//
...
const { canonicalize } = await import(
  path.resolve(here, '../../warden/dist/jcs.js'));
...
ok(verify(null, Buffer.from(payload, 'utf8'), pub, Buffer.from(doc.signature, 'hex')),
   "WARDEN's own canonicalizer + node:crypto accept the LIVE signature");
